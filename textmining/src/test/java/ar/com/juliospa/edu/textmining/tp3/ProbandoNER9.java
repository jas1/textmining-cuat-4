...
public class ProbandoNER9 {
	...
	public void filtrarThreadsTest(){
		...
		List<UrlGuardada> resultForos = gson.fromJson(json, new TypeToken<List<UrlGuardada>>(){}.getType());
		// recorrer threads
	...
	public void procesarThreadTestFecha1(){
		...
		List<UrlGuardada> resultThreads = gson.fromJson(jsonThread, new TypeToken<List<UrlGuardada>>(){}.getType());
		// recorrer threads
		Set<UrlGuardada> resultNers = patternRegexNer(resultThreads);

		System.out.println(gsonPretty.toJson(resultNers));
	 ...
	 * @return
	 */
	private Set<UrlGuardada> patternRegexNer(List<UrlGuardada> resultThreads) {
		List<UrlGuardada> res = new ArrayList<>();
		List<UrlGuardada> noClasificaFechas = new ArrayList<>();
		List<UrlGuardada> noClasificaUbicacion = new ArrayList<>();
		List<UrlGuardada> noClasificaEvento = new ArrayList<>();

		List<UrlGuardada> casosRaros = new ArrayList<>();
		

		// pattern 1 : fechas
		resultThreads.stream().forEach(el -> patternFecha(el, res, noClasificaFechas,casosRaros));
		noClasificaFechas.stream().forEach(notFecha -> log.warn("noClasificaFechas:\n"+gson.toJson(notFecha)));
		
		
		// pattern 2 : location @ al fin
		resultThreads.stream().forEach(el -> patternUbicacion(el, res, noClasificaUbicacion,casosRaros));
		noClasificaUbicacion.stream().forEach(notFecha -> log.warn("noClasificaUbicacion:\n"+gson.toJson(notFecha)));


		// pattern 3: que evento
		resultThreads.stream().forEach(el -> patternEvento(el, res, noClasificaEvento,casosRaros));
		noClasificaEvento.stream().forEach(notFecha -> log.warn("noClasificaEvento:\n"+gson.toJson(notFecha)));
		
		casosRaros.stream().forEach(notFecha -> log.warn("casosRaros:\n"+gson.toJson(notFecha)));
		
		Set<UrlGuardada> setResultNers = new HashSet<>(res);

		return setResultNers;
	}

	private void patternEvento(UrlGuardada el, List<UrlGuardada> res, List<UrlGuardada> noClasificaEvento,
			List<UrlGuardada> casosRaros) {
		//	ubicacion pattern 1: fecha - evento o quien @ ubicacion > - evento o quien @
	...
	private void patternUbicacion(UrlGuardada el, List<UrlGuardada> res, List<UrlGuardada> noClasificaUbicacion,
			List<UrlGuardada> casosRaros) {
	...
	private Date patternFechasMatching(String strToEval,String datePatternMatches,String regexPattern) throws MatcherTimesException, ParseException{
		...
		while (matcher.find()) {
			if (masDeUna) {
				throw new MatcherTimesException("|fecha matcher 2 veces o mas para:"+ datePatternMatches + ":"+strToEval); 
//				log.warn("|fecha matcher 2 veces o mas para:"+ datePatternMatches);
			}else{
	...
	private void patternFecha(UrlGuardada el, List<UrlGuardada> res, List<UrlGuardada> noClasificaFechas,
			List<UrlGuardada> casosRaros) {
		...
		boolean parsed = patternFechasResult(el, res, noClasificaFechas,casosRaros, datePatternMatches1, datePattern1Regex);
		...
		if (!parsed) {
			...
			parsed = patternFechasResult(el, res, noClasificaFechas,casosRaros, datePatternMatches2, datePattern2Regex);
		}
	...
	private boolean patternFechasResult(UrlGuardada el, List<UrlGuardada> res, List<UrlGuardada> noClasificaFechas,List<UrlGuardada> casosRaros,
			String datePatternMatches1, String datePattern1Regex) {
		boolean	ret=false;
		Date tmp1=null;
		try {
	...
	private List<UrlGuardada> procesarSubForos(UrlGuardada parentUrl, String currentUrl, String comentario)
		...
		indexThreads.stream()
				.forEach(el -> threads.add(new UrlGuardada(TipoUrl.SUBFORO, el.html(), el.text(),
						el.attr("href").replace("https://www.nightclubber.com.ar/", "")
								.replace("/?s=8655145fd7497833abd5c187fcbfacfb", ""),"parseSubforoInForum|"+comentario)));
	...
	private List<UrlGuardada> procesarPaginaIndice(UrlGuardada parentUrl, String currentUrl, String comentario)
		...
		indexThreads.stream()
				.forEach(el -> threads.add(new UrlGuardada(TipoUrl.THREAD, el.html(), el.text(),
						el.attr("href").replace("https://www.nightclubber.com.ar/", "")
								.replace("/?s=8655145fd7497833abd5c187fcbfacfb", ""),"parseThread|"+comentario)));
	...
	private List<UrlGuardada> searchEventosPasados(File input) throws IOException {
		...
		titulosForo.stream()
				.forEach(el -> retObj.add(new UrlGuardada(TipoUrl.FORO, el.html(), el.text(),
						el.attr("href").replace("https://www.nightclubber.com.ar/", "")
								.replace("/?s=8655145fd7497833abd5c187fcbfacfb", ""),"parseForo")));
		...
		titulosSubforos.stream()
				.forEach(el -> retObj.add(new UrlGuardada(TipoUrl.SUBFORO, el.html(), el.text(),
						el.attr("href").replace("https://www.nightclubber.com.ar/", "")
								.replace("/?s=8655145fd7497833abd5c187fcbfacfb", ""),"parseSubforo")));
